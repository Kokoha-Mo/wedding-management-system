...
public class ConsultationController {
    ...
    public ResponseEntity<Map<String, Object>> getAllConsultations() {
        ...
        if (authentication == null || !authentication.isAuthenticated()
            ...
            response.put("message", "尚未登入或憑證無效");
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(response);
        }

        // 2. 從 Token 取得員工 Email，並去資料庫查出這個員工
        String empEmail = (String) authentication.getPrincipal();
        Employee employee = employeeRepository.findByEmail(empEmail).orElse(null);

        // 3. 檢查他的部門 ID 是否為 7 (接待部)
        if (employee == null || employee.getDepartment() == null || employee.getDepartment().getId() != 7) {
            response.put("success", false);
            response.put("message", "權限不足，僅限接待部人員查看");
